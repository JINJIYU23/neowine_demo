...
export default function DetectionInfo() {
  ...
  const fetchDetections = async () => {
    try {
      const response = await fetch('http://localhost:8000/api/detections/latest?limit=10');
      if (response.ok) {
  ...
  // 위험도 레벨 매핑
  const getRiskLevel = (className: string): { level: 'LOW' | 'MEDI' | 'HIGH'; color: string } => {
    const lowerClass = String(className || '').toLowerCase();
    if (lowerClass.includes('ally tank')) return { level: 'LOW', color: 'var(--blue-color)' };
    if (lowerClass.includes('ally')) return { level: 'LOW', color: 'var(--blue-color)' };
    if (lowerClass.includes('enemy tank')) return { level: 'HIGH', color: 'var(--red-color)' };
    if (lowerClass.includes('enemy')) return { level: 'MEDI', color: 'var(--orange-color)' };
            ...
            return (
              <li key={detection.id}>
                <DetectionInfoDetail 
                  ...
                  coordinates={`${(detection.x || 0).toFixed(4)}, ${(detection.y || 0).toFixed(4)}`}
